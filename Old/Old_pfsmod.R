...
feedbackModel <- function (t, y, mu) {
  ...
  if(hfa == 1) dc_dot <- - (dc- ((qN[1]/qmax)*(D[1]/(sum(D))) + (qN[2]/qmax)*(D[2]/(sum(D)))))*sccr[1]
  else dc_dot <- - (S_a- (B[1]/sum(B)))*sccr[3]

  Growth <- pmin(gL * L /(kL + L), gN * N / (kN + N)) * B
  B_dot <-  Growth - m * B
  N_dot <- a * (S - N) - sum(qN / (rho * l_Root) * Growth) +
    sum(qN^2 / (QN * rho * l_Root) * alphaN * d * D * dc)
  D_dot <- m * B - qN/QN * d * D * dc
